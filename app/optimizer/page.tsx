...
export default function OptimizerPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("http://0.0.0.0:10000/api/optimize_routes", {
        method: "POST",
        body: formData,
      })
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      for (const busIndex of Object.keys(result)) {
        const busNumber = Number.parseInt(busIndex) + 1
        ...
        // Download HTML file
        handleDownload(busIndex, "html_file", `bus_${busNumber}_map.html`)
        await new Promise((resolve) => setTimeout(resolve, 300))

        // Download PNG file
        handleDownload(busIndex, "png_file", `bus_${busNumber}_map.png`)
        await new Promise((resolve) => setTimeout(resolve, 300))
      }
    } catch (error) {
              ...
              <CardContent>
                <Tabs defaultValue="bus0" className="w-full">
                  <TabsList className="bg-white/20 w-full">
                    {Object.keys(result).map((busIndex) => (
                      <TabsTrigger
                        key={busIndex}
                        value={`bus${busIndex}`}
                        className="text-white data-[state=active]:bg-indigo-600"
                      >
                        Bus {Number.parseInt(busIndex) + 1}
                      </TabsTrigger>
                    ))}
                  </TabsList>
                    ...
                    <TabsContent key={busIndex} value={`bus${busIndex}`} className="mt-4">
                      <div className="grid gap-4">
                        ...
                        <div className="flex items-center justify-between p-4 rounded-lg bg-white/5">
                          <div className="flex items-center">
                            <FileText className="h-5 w-5 text-white mr-3" />
                            <span className="text-white">Interactive Map (HTML)</span>
                          </div>
                          <Button
                            variant="outline"
                            size="sm"
                            className="bg-transparent border-white/30 text-white hover:bg-white/20"
                            onClick={() => handleDownload(busIndex, "html_file", result[busIndex].html_file)}
                          >
                            <Download className="h-4 w-4 mr-2" />
                            Download
                          </Button>
                        ...
                        <div className="flex items-center justify-between p-4 rounded-lg bg-white/5">
                          <div className="flex items-center">
                            <ImageIcon className="h-5 w-5 text-white mr-3" />
                            <span className="text-white">Route Map (PNG)</span>
                          </div>
                          <Button
                            variant="outline"
                            size="sm"
                            className="bg-transparent border-white/30 text-white hover:bg-white/20"
                            onClick={() => handleDownload(busIndex, "png_file", result[busIndex].png_file)}
                          >
                            <Download className="h-4 w-4 mr-2" />
                            Download
                          </Button>
